...
Usage:
    python get_environment_simple.py ENVIRONMENT_ID
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python get_environment_simple.py ENVIRONMENT_ID")
        sys.exit(1)
    ...
    print(f"🔍 Getting environment details: {environment_id}")
    ...
    try:
        ...
        if hasattr(environment, '_kwargs') and 'Environment' in environment._kwargs:
            ...
            print(f"   Raw response: {environment}")
