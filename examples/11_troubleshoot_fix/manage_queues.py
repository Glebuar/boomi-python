        ...
        for result in queue_results:
            if hasattr(result, 'queue_record') and result.queue_record:
                for queue_record in result.queue_record:
                    # Validate queue record has meaningful data 
                    # (SDK parsing bug creates fake records with single character values)
                    dead_letters_val = getattr(queue_record, 'dead_letters_count', None)
                    if isinstance(dead_letters_val, str) and len(dead_letters_val) == 1:
                        # Skip invalid records created by SDK parsing bug
                        continue
