...
class RuntimeRestartManager:
    ...
    def validate_health(self, atom_id: str) -> bool:
        ...
        # Check online status
        health_checks.append(('Online Status', status['online']))
        
        # Check version
        health_checks.append(('Version Available', status['version'] != 'N/A'))
        
        # Check capabilities
        has_capabilities = len(status.get('capabilities', [])) > 0
        health_checks.append(('Capabilities Loaded', has_capabilities))
        
        # Print health check results
        all_healthy = True
        for check_name, check_result in health_checks:
            icon = '✅' if check_result else '❌'
            print(f"   {icon} {check_name}: {'Passed' if check_result else 'Failed'}")
            if not check_result:
                all_healthy = False
